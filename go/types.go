// Package lotterypicker provides a Go client for the Lottery Number Generator API.
//
// For more information, visit: https://apiverve.com/marketplace/lotterypicker?utm_source=go&utm_medium=readme
package lotterypicker
...
// Request contains the parameters for the Lottery Number Generator API.
//
// Parameters:
//   - game: string - Lottery game preset
//   - numbers: integer - Number of main numbers to pick (for custom game) [min: 1, max: 20]
//   - max: integer - Maximum number for main numbers (for custom game) [min: 10, max: 100]
//   - bonus: integer - Number of bonus numbers (for custom game) [min: 0, max: 5]
//   - tickets: integer - Number of tickets to generate [min: 1, max: 25]
type Request struct {
	Game string `json:"game,omitempty"` // Optional
	Numbers int `json:"numbers,omitempty"` // Optional
	Max int `json:"max,omitempty"` // Optional
	Bonus int `json:"bonus,omitempty"` // Optional
	Tickets int `json:"tickets,omitempty"` // Optional
}
...
func (r *Request) Validate() error {
	rules := map[string]ValidationRule{
		...
		"numbers": {Type: "integer", Required: false, Min: float64Ptr(1), Max: float64Ptr(20)},
		"max": {Type: "integer", Required: false, Min: float64Ptr(10), Max: float64Ptr(100)},
		"bonus": {Type: "integer", Required: false, Min: float64Ptr(0), Max: float64Ptr(5)},
		"tickets": {Type: "integer", Required: false, Min: float64Ptr(1), Max: float64Ptr(25)},
	}
...
// ResponseData contains the data returned by the Lottery Number Generator API.
type ResponseData struct {
	Game string `json:"game"`
	GameType string `json:"game_type"`
	Config ConfigData `json:"config"`
	TicketsGenerated int `json:"tickets_generated"`
	Tickets []TicketsItem `json:"tickets"`
	AvailableGames []string `json:"available_games"`
}

// ConfigData represents the config object.
type ConfigData struct {
	MainNumbersCount int `json:"main_numbers_count"`
	MainNumbersRange string `json:"main_numbers_range"`
	BonusNumbersCount int `json:"bonus_numbers_count"`
	BonusNumbersRange string `json:"bonus_numbers_range"`
}

// TicketsItem represents an item in the Tickets array.
type TicketsItem struct {
	TicketNumber int `json:"ticket_number"`
	MainNumbers []int `json:"main_numbers"`
	BonusNumbers []int `json:"bonus_numbers"`
}
